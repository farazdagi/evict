use {
    evict::{EvictError, EvictionPolicy},
    std::sync::Arc,
};

pub fn basic_ops(replacer: Arc<impl EvictionPolicy<Error = EvictError>>) {
...
pub fn touch(replacer: Arc<impl EvictionPolicy<Error = EvictError>>) {
...
pub fn remove(replacer: Arc<impl EvictionPolicy<Error = EvictError>>) {
    // Scenario: unpin elements, i.e. add them to the replacer.
    replacer.unpin(1).unwrap();
    replacer.unpin(2).unwrap();
    replacer.unpin(3).unwrap();
